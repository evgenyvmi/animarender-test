...
class RabbitMQClient:
    ...
    def call(self, request_body, request_id):
        """
        ...
        LOGGER.info('Sending a request to RabbitMQ (ID: %s)', request_id)
        self._channel.basic_publish(
            exchange='',
            routing_key=self._server_queue,
            body=request_json,
    ...
    def _on_client_queue_declared(self, frame):
        ...
        LOGGER.info('Listening to "%s" queue', self._client_queue)
    ...
    def rpc(self, response):
        data = json.loads(response['args'][0])
        costs = []
        for element in data:
            #datet = element['completion_time'].replace('T','')
            completion_time = datetime.strptime(element['completion_time'], '%Y-%m-%dT%H:%M:%S')
            start_time = datetime.strptime(element['start_time'], '%Y-%m-%dT%H:%M:%S')

            total_seconds = (completion_time - start_time).total_seconds()

            costs.append({
                'id': element['id'],
                'cost': total_seconds / 3600 * element['nodes_used'] * element['passmark'] / 15000
            })
        return costs
    ...
    def _consumer_callback(self, channel, method, properties, body):
        ...
        what_to_do = response['method']

        my_str = 'self.' + what_to_do + '(response)'
        request = eval(my_str)
        request_json = json.dumps(request, sort_keys=True)
        self.call(request_json, response_id)
